...
@gql.type
class PlaceMutation:
    create_place: Place = gql.django.create_mutation(PlaceInput)
    update_place: Place = gql.django.update_mutation(PlaceInputPartial)
    delete_place: Place = gql.django.delete_mutation(gql.NodeInput)
    ...
    @gql.django.input_mutation
    def place_lock(self, info, place_id: gql.relay.GlobalID) -> Place:
        place = place_id.resolve_node(info)
        place = place.lock(info.context.user)
        return place

    @gql.django.input_mutation
    def place_release_lock(self, info, place_id: gql.relay.GlobalID) -> Place:
        place = place_id.resolve_node(info)
        place = place.release_lock(info.context.user)
        return place
